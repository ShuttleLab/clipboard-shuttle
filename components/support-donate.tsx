...
export function SupportDonateButton() {
  const { t } = useI18n();
  const donate = t.about?.donate as unknown as Record<string, string> | undefined;
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{donate.title}</DialogTitle>
          <DialogDescription>{donate.desc}</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <DialogClose asChild>
            <Button variant="secondary">{donate.close}</Button>
          </DialogClose>
        </DialogFooter>
      </DialogContent>
    </Dialog>
